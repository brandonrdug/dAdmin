...
:Callback(function(cmd, pl, args, suppress)
	...
	if (data.Rank == group) then
		da.sendcmderr(pl, cmd, '"' .. id .. '"\'s group is already "' .. group .. '"')
		return
	end
	...
	if not pl:IsSuperAdmin() and (pl:GetWeight() <= da.player.GetIDWeight(id)) then
		...
		if (targ) then
			targ:Message('# tried to set you to "#"')
				:Insert(pl)
				:Insert(group_data)
				:Send(DA_ERROR)
		end
	...
	if (targ) then
		da.groups.SetPlayerGroup(targ, group, expt, expto)
		...
		da.groups.SetSteamIDGroup(id:upper(), group, expt, expto)
		if not (suppress) then
			da.amsg('# set # to "#".#')
				:Insert(pl)
